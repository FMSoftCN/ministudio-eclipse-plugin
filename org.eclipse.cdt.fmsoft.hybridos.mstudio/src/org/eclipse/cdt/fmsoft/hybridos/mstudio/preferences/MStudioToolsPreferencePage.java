...
import org.eclipse.cdt.fmsoft.hybridos.mstudio.wizards.MStudioVersionWizard;
...
class MsVersionListener extends SelectionAdapter {
	...
	private MStudioToolsPreferencePage m_prefpage = null;

	public MsVersionListener(MStudioToolsPreferencePage prefpage, int control) {
		m_control = control;
		m_prefpage = prefpage;
	}
	...
	public void widgetSelected(SelectionEvent e) {
		if (m_control == ADD) {
			...
			MStudioVersionWizard versionWizard =
				new MStudioVersionWizard(MStudioMessages.getString("MStudioVersionWizard.edit.title"));
			WizardDialog versionDialog = new WizardDialog(m_prefpage.getShell(), versionWizard);
...
public class MStudioToolsPreferencePage extends PreferencePage
	...
	public MStudioToolsPreferencePage() {
	}
	...
	public static String[] getMStudioVersions() {
		...
		for (int i = 0; i < count; ++i) {
			String name = MStudioPreferenceConstants.MSVERSION_NAME + MSTPP_PIONT + Integer.toString(i);
	...
	public static String getMStudioBinPath(String version) {
		...
		String defaultVersionBinPath = MSTPP_EMPTY;
		int count = store.getInt(MStudioPreferenceConstants.MSVERSION_COUNT);
	...
	private void addMsBuildsSection(Composite parent) {
		...
		label.setText(MStudioMessages.getString("MStudioToolsPreferencePage.label"));
		...
		column.setText(MStudioMessages.getString("MStudioToolsPreferencePage.column.Name"));
		column.setWidth(150);
		column.setResizable(true);
		column = new TableColumn(table, SWT.LEFT);
		column.setText(MStudioMessages.getString("MStudioToolsPreferencePage.column.Path"));
		column.setResizable(true);
		column.setWidth(150);
		...
		addButton.setText(MStudioMessages.getString("MStudioToolsPreferencePage.addButton"));
		gridData = new GridData();
		gridData.horizontalAlignment = GridData.FILL;
		...
		editButton.setText(MStudioMessages.getString("MStudioToolsPreferencePage.editButton"));
		gridData = new GridData();
		gridData.horizontalAlignment = GridData.FILL;
		...
		removeButton.setText(MStudioMessages.getString("MStudioToolsPreferencePage.removeButton"));
		gridData = new GridData();
		gridData.horizontalAlignment = GridData.FILL;
		...
		defaultButton.setText(MStudioMessages.getString("MStudioToolsPreferencePage.defaultButton"));
		gridData = new GridData();
		gridData.horizontalAlignment = GridData.FILL;
	...
	public boolean performOk() {
		...
		MStudioProject[] mgProjects = new MStudioProject[pros.length];
		String[] oldBinPaths = new String[pros.length];

		for (int i = 0; i < pros.length; ++i) {
			mgProjects[i] = new MStudioProject(pros[i]);
		...
		store.setValue(MStudioPreferenceConstants.MSVERSION_COUNT, table.getItemCount());

		for (int i = 0; i < table.getItemCount(); ++i) {
			store.setValue(MStudioPreferenceConstants.MSVERSION_NAME + MSTPP_PIONT + Integer.toString(i),
					table.getItem(i).getText(0));
			store.setValue(MStudioPreferenceConstants.MSVERSION_BINPATH + MSTPP_PIONT + Integer.toString(i),
					table.getItem(i).getText(1));
		}

		// updates all the MiniGUI projects and collect projects that need rebuild
		Vector<MStudioProject> outdated = new Vector<MStudioProject>();
		...
		for (int i = 0; i < mgProjects.length; ++i) {
			...
			if ((mgProjects[i].getMStudioBinPath() == null && oldBinPaths[i] != null)
				|| (mgProjects[i].getMStudioBinPath() != null
					&& !mgProjects[i].getMStudioBinPath().equals(oldBinPaths[i]))) {
	...
	private void askForRebuild(final Vector<MStudioProject> projects) {

		Shell shell = PlatformUI.getWorkbench().getDisplay().getActiveShell();
		MessageDialog dialog = new MessageDialog(shell,
				MStudioMessages.getString("MStudioToolsPreferencePage.dialogTitle"),
				null,
				MStudioMessages.getString("MStudioToolsPreferencePage.dialogMessages"),
				MessageDialog.QUESTION, 
				new String[] { IDialogConstants.YES_LABEL, IDialogConstants.NO_LABEL},
				0);

		if (dialog.open() == 0) {
			WorkspaceJob rebuild = new WorkspaceJob(MSTPP_REBUILD) {
	    		public boolean belongsTo(Object family) {
	    			return ResourcesPlugin.FAMILY_MANUAL_BUILD.equals(family);
	    		}
				public IStatus runInWorkspace(IProgressMonitor monitor) {

					Iterator<MStudioProject> i = projects.iterator();
					while (i.hasNext()) {
                        MStudioProject project = (MStudioProject)i.next();
